...
export default function AdminsClient({
  ...
  const [newFieldErrors, setNewFieldErrors] = useState<Partial<Record<keyof UserInput, string>>>({});
  const [manageFieldErrors, setManageFieldErrors] = useState<Partial<Record<keyof UserInput, string>>>({});

  const { data, isLoading, mutate } = useSWR(
    ["admin-admins", page, search],
    () => getAdminUsersAction({ page, role: ["admin", "superadmin"], search }),
    {
      revalidateOnFocus: false,
      fallbackData: page === 1 && !search ? initialData : undefined,
    },
  );
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      ...
      toast.error(t("admin.adminDeletionFailed"));
    } finally {
      setIsDeleting(false);
    }
  };
                ...
                <div className="space-y-6 pt-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className={cn("h-14 rounded-2xl border-2 font-medium", newFieldErrors.full_name ? "border-destructive focus-visible:ring-destructive" : "")}
                      value={newUser.full_name}
                      onChange={(e) => {
                        setNewUser({ ...newUser, full_name: e.target.value })
                        if (newFieldErrors.full_name) setNewFieldErrors(prev => ({ ...prev, full_name: undefined }))
                      }}
                      disabled={isCreating}
                    />
                    {newFieldErrors.full_name && <p className="text-xs text-destructive">{newFieldErrors.full_name}</p>}
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">
                    ...
                    <Input
                      ...
                      className={cn("h-14 rounded-2xl border-2 font-medium", newFieldErrors.email ? "border-destructive focus-visible:ring-destructive" : "")}
                      value={newUser.email}
                      onChange={(e) => {
                        setNewUser({ ...newUser, email: e.target.value })
                        if (newFieldErrors.email) setNewFieldErrors(prev => ({ ...prev, email: undefined }))
                      }}
                      disabled={isCreating}
                    />
                    {newFieldErrors.email && <p className="text-xs text-destructive">{newFieldErrors.email}</p>}
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">
                      {t("admin.role")}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        type="password"
                        placeholder="••••••••"
                        className={cn("h-14 pl-12 rounded-2xl border-2 font-medium", newFieldErrors.password ? "border-destructive focus-visible:ring-destructive" : "")}
                        value={newUser.password}
                        onChange={(e) => {
                          setNewUser({ ...newUser, password: e.target.value })
                          if (newFieldErrors.password) setNewFieldErrors(prev => ({ ...prev, password: undefined }))
                        }}
                        disabled={isCreating}
                      />
                    </div>
                    {newFieldErrors.password && <p className="text-xs text-destructive">{newFieldErrors.password}</p>}
                  </div>
                </div>
          ...
          {managingUser && (
            <div className="space-y-6 pt-4">
              <div className="space-y-2">
                ...
                <Input
                  type="email"
                  className={cn("h-14 rounded-2xl border-2 font-medium", manageFieldErrors.email ? "border-destructive focus-visible:ring-destructive" : "")}
                  value={managingUser.email || ""}
                  onChange={(e) => {
                    setManagingUser({ ...managingUser, email: e.target.value })
                    if (manageFieldErrors.email) setManageFieldErrors(prev => ({ ...prev, email: undefined }))
                  }}
                  disabled={isUpdating}
                />
                {manageFieldErrors.email && <p className="text-xs text-destructive">{manageFieldErrors.email}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">
                  {t("admin.fullName")}
                ...
                <Input
                  className={cn("h-14 rounded-2xl border-2 font-medium", manageFieldErrors.full_name ? "border-destructive focus-visible:ring-destructive" : "")}
                  ...
                  onChange={(e) => {
                    ...
                    if (manageFieldErrors.full_name) setManageFieldErrors(prev => ({ ...prev, full_name: undefined }))
                  }}
                  disabled={isUpdating}
                />
                {manageFieldErrors.full_name && <p className="text-xs text-destructive">{manageFieldErrors.full_name}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">
                  {t("admin.role")}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={cn("h-14 pl-12 rounded-2xl border-2 font-medium", manageFieldErrors.password ? "border-destructive focus-visible:ring-destructive" : "")}
                    ...
                    onChange={(e) => {
                      ...
                      if (manageFieldErrors.password) setManageFieldErrors(prev => ({ ...prev, password: undefined }))
                    }}
                    disabled={isUpdating}
                  />
                </div>
                {manageFieldErrors.password && <p className="text-xs text-destructive">{manageFieldErrors.password}</p>}
              </div>
            </div>
          )}
          <DialogFooter className="mt-8">
